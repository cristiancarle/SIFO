...
from modulos.licencia import estado_licencia, solicitar_activacion_por_consola, solicitar_activacion_por_gui
from core.analizador import analizar_incendio, analizar_coordenadas, validar_coordenadas
...
def opcion_registrar():
    ...
    valido, error = validar_coordenadas(latitud, longitud)
    if not valido:
        print(f"\n{error}")
        return
...
def validar_licencia_de_arranque() -> bool:
    ...
    try:
        import tkinter
        return solicitar_activacion_por_gui()
    except Exception:
        return solicitar_activacion_por_consola()


def main():
    parser = argparse.ArgumentParser(description="SIFO - Sistema Inteligente para Incendios Forestales")
    parser.add_argument("--cli", action="store_true", help="Inicia directamente en el menú de línea de comandos.")
    parser.add_argument("--gui", action="store_true", help="Inicia directamente en la interfaz gráfica.")
    args = parser.parse_args()

    crear_carpetas()
    crear_base_datos()

    if not validar_licencia_de_arranque():
        print("\nLa aplicación queda bloqueada hasta que la licencia sea activada.")
        sys.exit(1)
    ...
    if args.gui or not sys.argv[1:]:
        print("\nIniciando interfaz gráfica...")
        if not iniciar_interfaz_grafica():
            print("\nLa interfaz gráfica no pudo iniciarse. Se abrirá el menú de línea de comandos.")
            menu_cli()
        return
