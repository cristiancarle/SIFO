...
class TestGenerarInformePDF(unittest.TestCase):
    ...
    def test_generar_estrategia_noche_con_pendiente_y_humedad_baja(self):
        meteo = {
            'temperatura': 3.3,
            'humedad': 10,
            'viento': 8,
            'categoria_viento': 'Bajo',
            'is_day': False,
        }
        topografia = {'maxima_pendiente_pct': 28}
        combustible = {'tipo_combustible': 'Bosque'}

        estrategias = _generar_estrategia_periodo(meteo, topografia, combustible)
        self.assertTrue(any('noche' in e.lower() or 'nocturna' in e.lower() for e in estrategias))
        self.assertTrue(any('máxima alerta' in e.lower() or 'muy seco' in e.lower() for e in estrategias))
        self.assertTrue(any('pendiente' in e.lower() or 'descend' in e.lower() for e in estrategias))
