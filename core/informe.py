...
def generar_informe(analisis):
    ...
    lineas.append(f"Fecha: {incendio['fecha'] or fecha_confeccion.split()[0]} {incendio['hora'] or fecha_confeccion.split()[1]}")
    lineas.append(f"Fecha de confección: {fecha_confeccion}")
    ...
    riesgo = analisis.get("riesgo") or {}
    prediccion = analisis.get("prediccion") or {}
    analisis_ia = analisis.get("analisis_ia") or analisis.get("ia") or {}
    if riesgo:
        lineas.append("Nivel de riesgo general:")
        lineas.append(f"- {riesgo.get('nivel', 'N/A')} ({riesgo.get('puntuacion', 'N/A')}/100)")
        lineas.append(f"- Descripción: {riesgo.get('descripcion', 'N/A')}")
        lineas.append("")

    if analisis_ia:
        lineas.append("Análisis IA de propagación:")
        lineas.append(
            f"- Dirección probable: {analisis_ia.get('direccion_probable_cardinal', 'N/A')} "
            f"({analisis_ia.get('direccion_probable_grados', 'N/A')}°)"
        )
        lineas.append(
            f"- Aceleración estimada: {analisis_ia.get('nivel_aceeleracion', 'N/A')} "
            f"(factor {analisis_ia.get('factor_aceeleracion', 'N/A')})"
        )
        lineas.append(f"- Observación IA: {analisis_ia.get('respuesta_ia', analisis_ia.get('descripcion', 'N/A'))}")
        lineas.append("")

    if prediccion:
        lineas.append("Predicción de propagación del incendio:")
        lineas.append(
            f"- Velocidad estimada: {prediccion.get('velocidad', 'N/A')} "
            f"(factor {prediccion.get('factor', 'N/A')})"
        )
        lineas.append(f"- Humedad del combustible: {prediccion.get('humedad_combustible', 'N/A')} %")
        lineas.append(f"- Observación: {prediccion.get('descripcion', 'N/A')}")
        lineas.append("")
    ...
    lineas.append(f"- Humedad del combustible: {combustible.get('humedad_combustible', 'N/A')} %")
    ...
    for recomendacion in analisis.get("recomendaciones", []):
