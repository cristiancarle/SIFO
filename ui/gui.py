...
class SIFOApp(tk.Tk):
    def __init__(self):
        ...
        ancho = max(1100, int(screen_w * 0.88))
        alto = max(780, int(screen_h * 0.90))
        self.geometry(f"{ancho}x{alto}")
        self.minsize(int(screen_w * 0.65), int(screen_h * 0.65))
    ...
    def _create_tab_analizar(self):
        ...
        self.var_opcion.trace_add('write', actualizar_visibilidad)
