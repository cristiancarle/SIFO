...
def crear_grafico_meteorologia(datos_json, frame_padre):
    ...
    try:
        ...
        pronostico = datos.get('pronostico_24h', {}).get('datos_horarios', [])
        ...
        for dato in pronostico:
            hora = datetime.fromisoformat(dato['hora'].replace('Z', '+00:00')).strftime('%H:%M')
            horas.append(hora)
            temperaturas.append(float(dato.get('temperatura', 0)))
            humedad.append(float(dato.get('humedad', 0)))
            velocidad_viento.append(float(dato.get('velocidad_viento', 0)))
